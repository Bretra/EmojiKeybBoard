...
+ (instancetype)keyBoardWithNavgationBarTranslucent:(BOOL)translucent
{
    ...
    if (translucent) {
        frame = CGRectMake(0, kScreenHeight - kChatToolBarHeight - KMagrinBottom, kScreenWidth, kChatKeyBoardHeight);
    }else {
        frame = CGRectMake(0, kScreenHeight - kChatToolBarHeight - KStatusBarAndNavigationBarHeight - KMagrinBottom, kScreenWidth, kChatKeyBoardHeight);
    }
...
{
    CGRect frame = CGRectMake(0, bounds.size.height - kChatToolBarHeight - KMagrinBottom, kScreenWidth, kChatKeyBoardHeight);
    return [[self alloc] initWithFrame:frame];
}
...
//        [self initialHiddenSubview:YES];
    }
    return self;
}

- (void)initialHiddenSubview:(BOOL)hidden {
    __block CGRect facePanelFrame = self.facePanel.frame;
    __block CGRect morePanelFrame = self.morePanel.frame;
    
    [UIView animateWithDuration:0.25 animations:^{
        facePanelFrame.origin.y = self.chatToolBar.frame.size.height + KMagrinBottom;
        morePanelFrame.origin.y = self.chatToolBar.frame.size.height + KMagrinBottom;
        self.facePanel.frame = facePanelFrame;
        self.morePanel.frame = morePanelFrame;
    }];
    
//    self.facePanel.hidden = hidden;
//    self.morePanel.hidden = hidden;
//    self.OAtoolbar.hidden = hidden;
}
...
{    
    ...
    if (self.chatToolBar.faceSelected)
    {
        [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            self.facePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame)-kFacePanelHeight - KMagrinBottom, CGRectGetWidth(self.frame), kFacePanelHeight);
    ...
    else if (self.chatToolBar.moreFuncSelected)
    {
        [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            self.lastChatKeyboardY = self.frame.origin.y;
            self.frame = CGRectMake(0, [self getSuperViewH]-CGRectGetHeight(self.frame), kScreenWidth, CGRectGetHeight(self.frame));
            self.morePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame)-kFacePanelHeight - KMagrinBottom, CGRectGetWidth(self.frame), kFacePanelHeight);
        ...
        [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            self.lastChatKeyboardY = self.frame.origin.y;
            self.frame = CGRectMake(0, [self getSuperViewH]- kChatToolBarHeight - KMagrinBottom, kScreenWidth, CGRectGetHeight(self.frame));
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            CGFloat chatToolBarHeight = CGRectGetHeight(self.frame) - kMorePanelHeight - KMagrinBottom;
...
- (void)updateAssociateTableViewFrame
{
    ...
    if (fabs(tableViewContentDiffer) > fabs(keyboardOffset)) {
        offset = original-keyboardOffset;//升
    }else {
        if (self.chatToolBar.voiceSelected || self.chatToolBar.faceSelected || self.chatToolBar.moreFuncSelected) {
            offset = tableViewContentDiffer;
        }else {
            offset = tableViewContentDiffer + KMagrinBottom;//降
        }
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    if (object == self && [keyPath isEqualToString:@"self.chatToolBar.frame"]) {
        ...
        self.facePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame)-kFacePanelHeight - KMagrinBottom, CGRectGetWidth(self.frame), kFacePanelHeight);
        self.morePanel.frame = CGRectMake(0, CGRectGetHeight(self.frame)-kMorePanelHeight - KMagrinBottom, CGRectGetWidth(self.frame), kMorePanelHeight);
...
- (void)chatToolBar:(ChatToolBar *)toolBar voiceBtnPressed:(BOOL)select keyBoardState:(BOOL)change
{
    if (select && change == NO) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            y = [self getSuperViewH] - self.chatToolBar.frame.size.height - KMagrinBottom;
...
- (void)setKeyBoardStyle:(KeyBoardStyle)keyBoardStyle
{
    ...
    if (keyBoardStyle == KeyBoardStyleComment) {
        self.lastChatKeyboardY = self.frame.origin.y;
        self.frame = CGRectMake(0, self.frame.origin.y+kChatToolBarHeight + KMagrinBottom, self.frame.size.width, self.frame.size.height);
    }
}
    ...
    if (self.keyBoardStyle == KeyBoardStyleChat)
    {
        if ([self.chatToolBar.textView isFirstResponder])
        {
            ...
            [UIView animateWithDuration:0.25 animations:^{
                ...
                if (K_iPhoneXStyle) {
                    selfFrame.origin.y = [self getSuperViewH] - kChatToolBarHeight - KMagrinBottom;
...
//                    CGRect facePaFrame = self.facePanel.frame;
//                    facePaFrame.origin.y = self.chatToolBar.frame.size.height + KMagrinBottom;
//                    self.facePanel.frame = facePaFrame;
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    BOOL containsStatus = CGRectContainsPoint(self.bounds, point);
