...
- (void)initSubViews
{
    ...
    //change
    _addBtn.hidden = YES;
    _facePickerView.frame = CGRectMake(10, 0, kScreenWidth - kFacePanelBottomToolBarHeight - 10 - 10, kFacePanelBottomToolBarHeight);
    //
    
    
    _sendBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    _sendBtn.frame = CGRectMake(kScreenWidth-kFacePanelBottomToolBarHeight - 10, 0, kFacePanelBottomToolBarHeight, [UIFont systemFontOfSize:16].lineHeight + 5);
    _sendBtn.center = CGPointMake(_sendBtn.center.x, self.frame.size.height / 2);
...
- (void)loadfaceThemePickerSource:(NSArray *)pickerSource
{
    for (int i = 0; i<pickerSource.count; i++) {
        ...
        btn.tag = i+100;
        [btn setImage:[UIImage imageNamed:@"emojiIcon"] forState:UIControlStateNormal];
...
{
    if (_facePickerView.contentSize.width > _facePickerView.frame.size.width) {
