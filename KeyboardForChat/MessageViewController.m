...
@property (nonatomic, strong) NSMutableAttributedString *att;
...
- (void)viewDidLoad {
    ...
    UITableView *messageTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-49 - KMagrinBottom) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    style.lineSpacing = 6;
    NSMutableAttributedString *attttt = [[NSMutableAttributedString alloc] initWithAttributedString:self.att];
    [attttt addAttributes:@{NSParagraphStyleAttributeName:style} range:NSMakeRange(0, self.att.length)];
    cell.detailTextLabel.attributedText = attttt;
...
- (void)chatKeyBoardSendText:(NSString *)text {
   RegularEmojiManager *emojiManager = [[RegularEmojiManager alloc] init];
   self.att = [emojiManager regularSpecialCharsWithCovertString:text andFont:[UIFont systemFontOfSize:18]];
//    self.att = [emojiManager regularEmojiWithString:text];
    NSIndexPath *index = [NSIndexPath indexPathForRow:19 inSection:0];
    [self.messageTableView reloadRowsAtIndexPaths:@[index] withRowAnimation:UITableViewRowAnimationNone];
    NSLog(@"%@", emojiManager);
}
